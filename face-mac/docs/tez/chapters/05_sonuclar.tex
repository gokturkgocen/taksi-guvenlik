...
\paragraph{ESP32-CAM $\rightarrow$ EC2.} Uçtan uca burst akışı, sergi
öncesi yapılan tezgah testlerinde doğrulanmıştır: ESP32-CAM Wi-Fi STA
modunda hotspot'a bağlanmakta, 10 kareyi sunucuya POST etmekte ve JSON
yanıtı parse etmektedir. Server tarafındaki yapılandırılmış JSON günlüğü,
gelen burst sayısını, kalite-geçen kare sayısını ve gecikmeyi izlemekte
kullanılmıştır.

\paragraph{STM32 bring-up.} CubeMX projesi açılmış, build geçmiş ve
kart flash edilmiştir. Onboard LED (LD1/LD2/LD3) GPIO toggle testleri
yapılmıştır. Bu tezin yazıldığı tarihte; STM32 ile ESP32-CAM'in birlikte
ayağa kaldırılması ve UART köprüsünün uçtan uca test edilmesi planlanmış
ancak henüz tamamlanmamıştır. State machine modülü ise host-side bir
test harness'ında 12 senaryo için doğrulanmıştır (\Cref{tab:host-tests});
bu test seti, donanım üzerinde flash + flash döngülerinin sayısını
azaltmıştır.

\paragraph{HM-10 BLE.} HM-10 modülü, telefon ve nRF Connect uygulaması
kullanılarak tek başına test edilmiştir. \texttt{0xFFE0} servis ve
\texttt{0xFFE1} karakteristik üzerinden okuma--yazma çalışmış; modülün
BLE tarafı sağlam bulunmuştur. STM32 UART2'ye bağlanması, durum geçiş
satırlarının BLE notification olarak telefona iletilmesi sergi
hazırlığının bir parçasıdır.

\paragraph{Android.} Uygulama iskeleti, donanım gelmeden önce yazılıp
test edilmiştir. ``DEV: Simulate MATCH'' butonu üzerinden, sahte bir
\texttt{MATCH:Test\_Subject;0.95} satırı parse pipeline'a beslenmekte
ve telefon otomatik olarak 155 hattını aramaktadır
(\texttt{Intent.ACTION\_CALL}).
