...
\begin{frame}{Uçtan Uca Mimari}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={draw=ink, thick, minimum width=26mm, minimum height=10mm,
                    align=center, font=\scriptsize\sffamily},
        cloud/.style={draw=ink, thick, minimum width=32mm, minimum height=10mm,
    ...
    \node[box] (esp) {ESP32-CAM\\\tiny burst + Wi-Fi + BLE};
    \node[box, below=of esp] (stm) {STM32 F767ZI\\\tiny olay yöneticisi};
    \node[box, right=of esp] (wifi) {Wi-Fi};
    \node[cloud, right=of wifi] (ec2) {AWS EC2\\\tiny Flask + InsightFace + Auth};
    \node[box, below=of ec2] (phone) {iPhone\\\tiny SwiftUI · tel:// dialer};
    ...
    \draw[arr] (esp) -- node[lab, right]{USART6} (stm);
    \draw[arr] (esp.south east) to[bend left=20]
        node[lab, right]{BLE FFE0/FFE1} (phone.north);
    \draw[arracc] (phone.north west) to[bend left=20]
        node[lab, left]{/auth} (ec2.south west);
    \draw[arrsig] (phone.south) -- ++(0,-6mm) node[lab, right]{tel:// 155};
    \end{tikzpicture}
    \end{center}
...
\begin{frame}{Kullanıcı Yönetimi Akışı}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={draw=ink, thick, minimum width=24mm, minimum height=8mm,
                    align=center, font=\scriptsize\sffamily},
        arr/.style={-{Latex[length=2mm]}, thick, ink, font=\tiny\ttfamily},
    ]
    \node[box] (open) {Uygulama açılır};
    \node[box, right=of open] (boot) {Keychain'de\\token var mı?};
    \node[box, above right=2mm and 10mm of boot] (verify) {\texttt{/auth/me}};
    \node[box, below right=2mm and 10mm of boot] (login) {Login};
    \node[box, right=of verify] (home) {HomeView\\(TabView)};
    \node[box, right=of login] (reg) {Register\\(otomatik login)};
